...
bool EvolutionResultAnimationLayer::init()
{
    if ( !Layer::init() )
    {
        return false;
    }
    HMSLOG("EvolutionResultAnimationLayer::init()");
    initEventListener();
...
void EvolutionResultAnimationLayer::setData(int gacha_type,int card_count,int hit_card[],int rare_card[],int chara_type[], int chara_list_id[]){
...
void EvolutionResultAnimationLayer::getSize(){
...
void EvolutionResultAnimationLayer::createBg(){
...
void EvolutionResultAnimationLayer::setCardByIndex(int index){
    //裏のカード
...
void EvolutionResultAnimationLayer::setGachaInfo(){
	if(!info_flg) {
...
void EvolutionResultAnimationLayer::updateForResultFlow(float dt){
    //スケジュールを削除
    this->unschedule(schedule_selector(EvolutionResultAnimationLayer::updateForResultFlow));
    se_bool=true;
...
void EvolutionResultAnimationLayer::resultActionForceEnd(int card_index){
    //全てのアクションを全て強制終了する
    _card_back->stopAllActions() ;
    _card_open->stopAllActions() ;
...
void EvolutionResultAnimationLayer::resultActionByCardIndex(int card_index){
    ...
    _chara_detail_sprite->runAction(Sequence::create(DelayTime::create(dark_end_time), DelayTime::create(zoom_time), DelayTime::create(rotation_90_time*2.0),DelayTime::create(after_rotation_blank),CallFunc::create(CC_CALLBACK_0(EvolutionResultAnimationLayer::deleteRarityAction,this)), detail_show_action_for_chara_detail, now_anim_false_func, nullptr)) ;
    
}

void EvolutionResultAnimationLayer::effect(){
    //ガチャアニメーションエフェクト音(１回)
    if(se_bool) CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("download/SE/slash3.mp3",false);
    se_bool=false;
...
void EvolutionResultAnimationLayer::deleteRarityAction(){
        for(Node *node : getChildren()){
            if(node->getTag() == TAG_RERITY || node->getTag() == TAG_EFFECT){
...
void EvolutionResultAnimationLayer::popupRarity(int card_index){
...
void EvolutionResultAnimationLayer::animationEnd(){
    /*
    if(touch_end){
        allTapDisaleRemove();
    ...
    if(_animation_end == false){
        if(_card_count==1){
            ...
            this->runAction(Sequence::create(DelayTime::create(0.5f), CallFunc::create(CC_CALLBACK_0(EvolutionResultAnimationLayer::allTapDisaleRemove,this)), RemoveSelf::create(), nullptr)) ;
        }
...
void EvolutionResultAnimationLayer::setScheduler(Layer *parent, SEL_SCHEDULE sel)
{
    _parent = parent;
    _sel = sel;
}

void EvolutionResultAnimationLayer::allTapDisaleRemove(){
...
void EvolutionResultAnimationLayer::createColorSprite(){
    //真っ白な画面いっぱいのスプライト
...
void EvolutionResultAnimationLayer::onEnter(){
    ...
    //リザルトのアニメーションのフロー開始
    this->schedule(schedule_selector(EvolutionResultAnimationLayer::updateForResultFlow), 2.5);
    
}

void EvolutionResultAnimationLayer::setCharaDetailByIndex(int index){
...
void EvolutionResultAnimationLayer::initEventListener()
{
    //タップイベントの設定
    listener = EventListenerTouchOneByOne::create();
    listener->onTouchBegan = CC_CALLBACK_2(EvolutionResultAnimationLayer::onTouchBegan, this);
    listener->onTouchMoved = CC_CALLBACK_2(EvolutionResultAnimationLayer::onTouchMoved, this);
    listener->onTouchEnded = CC_CALLBACK_2(EvolutionResultAnimationLayer::onTouchEnded, this);
    listener->onTouchCancelled = CC_CALLBACK_2(EvolutionResultAnimationLayer::onTouchCancelled, this);
    listener->setSwallowTouches(true);

    //タップイベントの登録
    _eventDispatcher->addEventListenerWithSceneGraphPriority(listener, this);
}
bool EvolutionResultAnimationLayer::onTouchBegan(cocos2d::Touch *touch, cocos2d::Event *event)
{
    if(_touch_enable == false) return false  ;
    
    //一定間隔タッチ不可能にする
    _touch_enable = false ;
    this->schedule(schedule_selector(EvolutionResultAnimationLayer::touchIntervalCount), 0.3);
    
    return true ;
}
void EvolutionResultAnimationLayer::onTouchMoved(cocos2d::Touch *touch, cocos2d::Event *event)
{
}
void EvolutionResultAnimationLayer::onTouchEnded(cocos2d::Touch *touch, cocos2d::Event *event)
{
...
void EvolutionResultAnimationLayer::touchIntervalCount(float dt){
    _touch_enable = true;
}

void EvolutionResultAnimationLayer::onTouchCancelled(cocos2d::Touch *touch, cocos2d::Event *event)
{
}
