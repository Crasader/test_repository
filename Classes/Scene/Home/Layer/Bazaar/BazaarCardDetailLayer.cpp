...
BazaarCardDetailLayer::BazaarCardDetailLayer()
:_skill_status(SkillStatus::None)
,_close_func(nullptr)
{
}

BazaarCardDetailLayer* BazaarCardDetailLayer::create(string chara_id, string chara_list_id)
{
    BazaarCardDetailLayer *pRet = new BazaarCardDetailLayer();
...
bool BazaarCardDetailLayer::init(string chara_id, string chara_list_id)
{
    if ( !Layer::init() )
    {
        return false;
    }
    HMSLOG("BazaarCardDetailLayer::init()");
...
void BazaarCardDetailLayer::onEnter()
{
    Layer::onEnter() ;
...
void BazaarCardDetailLayer::initEventListener()
{
    //タップイベントの登録
...
void BazaarCardDetailLayer::getCharaDataRequest()
{
    //キャラデータ取得通信
    string url = HOST;
    url += "user/get_chara_data.php";
    auto selector = httpresponse_selector(BazaarCardDetailLayer::getCharaDataCallback);
    
    string body = "chara_id=" + _chara_id;
    HttpConnect::sendHttpRequest(this, url, selector, body);
}

void BazaarCardDetailLayer::getCharaDataCallback(HttpClient *sender, HttpResponse *response)
{
...
void BazaarCardDetailLayer::setCharaData(json11::Json json)
{
    //キャラデータの設定
    _chara_data = json["chara_data"].object_items();
...
void BazaarCardDetailLayer::createBg()
{
    //影
...
void BazaarCardDetailLayer::createNameLabel()
{
    //名前画像
    ...
    name->setPosition(Point(_bg->getContentSize().width / 2, 130 + 81));
    name->setScale(0.89);
    ...
    if (1 < fromString<int>(_rare) && _name_kana != "")
    {
        ...
        if (second_label) second_label->setScale(0.73f);
        
        //フリガナ
        auto kana_label = HMSFontUtil::createWithTTFBySystemFontArgument(_name_kana.c_str(), FONT_NAME_SYSTEM, 18);
        kana_label->setPosition(Point(_bg->getContentSize().width / 2, (130 + 123)));
        kana_label->setScale(0.89);
        _bg->addChild(kana_label, 5);
    }
}

void BazaarCardDetailLayer::createCharaStatus()
{
    //タイプアイコン
    ...
    icon_type->setPosition(Point(32, 67+86));
    icon_type->setAnchorPoint(Vec2(0, 0));
    icon_type->setScale(0.89);
    ...
    float power_label_right_x = 164;
    if(_chara_type == CharaSprite::CharaType::STRATEGIST) power_label_right_x=214;
    else if(_chara_type == CharaSprite::CharaType::GENERAL) power_label_right_x=144;
    ...
    laPowerLabel->setPosition(Vec2(power_label_right_x, 92 + 84));
    laPowerLabel->setScale(0.89);
    ...
    powerLabel->setPosition(Vec2(power_label_right_x + 5, 92+84));
    powerLabel->setScale(0.89);
    ...
    laCostLabel->setPosition(Vec2(421, 92+84));
    ...
    costLabel->setPosition(Vec2(426, 92+84));
    costLabel->setScale(0.89);
    _bg->addChild(costLabel, Z_Cost, T_Cost);
}

void BazaarCardDetailLayer::showSkill()
{
    //スキルの表示
    auto skill_sp_off = Sprite::create("init/CharaMenu/tab_p.png");
    auto skill_sp_on  = Sprite::create("init/CharaMenu/tab_p.png");
    
    auto menuItem = MenuItemSprite::create(skill_sp_off, skill_sp_on, CC_CALLBACK_1(BazaarCardDetailLayer::changeSkillDetail, this));
    menuItem->setScale(0.89);
    menuItem->setTag(T_Skill_MenuItem);
    
    auto menu = Menu::create(menuItem, nullptr);
    menu->setPosition(Point(_bg->getContentSize().width / 2, 168-88));
    ...
    if (fromString<int>(_passive_skill_id) > 0 && fromString<int>(_active_skill_id) > 0) {
        ...
        skill_tab->setPosition(Point(390, 112));
        skill_tab->setScale(0.89);
        _bg->addChild(skill_tab, Z_Skill_Tab, T_Skill_Tab);
        
        //切り替えボタンの作成
        auto switch_btn = MenuItemImage::create("init/CharaMenu/chara_detail_p_a.png", "init/CharaMenu/chara_detail_p_a.png", CC_CALLBACK_1(BazaarCardDetailLayer::changeSkillDetail, this));
        switch_btn->setScale(0.89);
        auto menu = Menu::create(switch_btn, nullptr);
        menu->setPosition(Point(448, 294));
    ...
    _skill_name->setPosition(Point(48, 92));
    _skill_name->setScale(0.89);
    ...
    _skill_detail->setPosition(Point(_bg->getContentSize().width / 2, 42));
    _skill_detail->setScale(0.89);
    ...
    _skill_text->setPosition(Point(390, 92));
    _skill_text->setScale(0.89);
    ...
    _skill_need_cost->setPosition(Point(470, 92));
    _skill_need_cost->setScale(0.89);
    _bg->addChild(_skill_need_cost, Z_Skill_Need_Cost, T_Skill_Need_Cost);
}

void BazaarCardDetailLayer::showCharaComment()
{
    //キャラ説明の表示
...
void BazaarCardDetailLayer::changeSkillDetail(Ref* sender)
{
    //両方持ちだったら切り替える処理
        ...
        switch (_skill_status) {
            ...
            case SkillStatus::Passive_Skill:
            {
                ...
                if (skill_tab) {
                    ...
                    skill_tab->setPosition(Point(145, 112));
                }
