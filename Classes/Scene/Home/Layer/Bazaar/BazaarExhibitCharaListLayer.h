#ifndef __war__BazaarExhibitCharaListLayer__
#define __war__BazaarExhibitCharaListLayer__
...
class BazaarExhibitCharaListLayer : public Layer , public ScrollViewDelegate
{
private:
...
public:
    BazaarExhibitCharaListLayer(); //コンストラクタ
    
    float ratio;
    
    static BazaarExhibitCharaListLayer* create(bool *chara_type, bool *chara_rarity);
    virtual bool init(bool *chara_type, bool *chara_rarity);
    //    CREATE_FUNC(BazaarExhibitCharaListLayer);
    ...
    void checkBanExhibitChara(); //出品不可能キャラチェック
    ...
    static void initRarityAndType(); //絞り込み状態を保存
    void saveRarityAndType(); //絞り込み状態を保存
...
#endif /* defined(__war__BazaarExhibitCharaListLayer__) */
