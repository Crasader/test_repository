...
DailyMissionClearAnimLayer* DailyMissionClearAnimLayer::create(string reward_item_type, string reward_related_id, string comment, string count, string chara_name, bool all_complate_anim_flg)
{
    DailyMissionClearAnimLayer *node = new DailyMissionClearAnimLayer();
...
bool DailyMissionClearAnimLayer::init(string reward_item_type, string reward_related_id, string comment, string count, string chara_name, bool all_complate_anim_flg)
{
    if ( !Layer::init() )
    {
        return false;
    }
    HMSLOG("DailyMissionClearAnimLayer::init()");
...
void DailyMissionClearAnimLayer::onEnter()
{
    Layer::onEnter();
...
void DailyMissionClearAnimLayer::createRunAction()
{
    //アクションフラグ
    _action_flg = true;
...
//        auto ease_reward = EaseBounceOut::create(move_reward);
        ...
        //ナビキャラの表示
        auto move_chara = MoveTo::create(0.2f, convertFromDisplaySize(Point(-50, 0) ,0.5 ,0));
        auto ease_chara = EaseOut::create(move_chara, 2);
...
void DailyMissionClearAnimLayer::createSprite()
{
    //背景画像の作成
    createBg();
...
void DailyMissionClearAnimLayer::initEventListener()
{
    //タップイベントの設定
    listener = EventListenerTouchOneByOne::create();
    listener->setSwallowTouches(true);
    listener->onTouchBegan = CC_CALLBACK_2(DailyMissionClearAnimLayer::onTouchBegan, this);
    listener->onTouchMoved = CC_CALLBACK_2(DailyMissionClearAnimLayer::onTouchMoved, this);
    listener->onTouchEnded = CC_CALLBACK_2(DailyMissionClearAnimLayer::onTouchEnded, this);
    listener->onTouchCancelled = CC_CALLBACK_2(DailyMissionClearAnimLayer::onTouchCancelled, this);
...
void DailyMissionClearAnimLayer::createNaviBaloon()
{
    if(_all_complate_anim_flg){
    }else{
        //吹き出しの作成
        _navi_balloon = Sprite::create("init/Home/DailyMission/Clear/navi_balloon.png") ;
...
void DailyMissionClearAnimLayer::createConfetti()
{
    if(_all_complate_anim_flg){
        //紙吹雪の生成
        _confetti_1 = Sprite::create("init/Home/DailyMission/Clear/confetti.png") ;
...
//        _confetti_1->setPosition(Point(_confetti_1->getContentSize() / 2));
        ...
        //紙吹雪の生成
        _confetti_2 = Sprite::create("init/Home/DailyMission/Clear/confetti.png") ;
...
//        _confetti_2->setPosition(Point(_confetti_2->getContentSize() / 2));
        ...
        schedule(schedule_selector(DailyMissionClearAnimLayer::moveConfetti), 0.01f);
    }
}

void DailyMissionClearAnimLayer::moveConfetti(float dt)
{
...
void DailyMissionClearAnimLayer::createLight()
{
    //ライトの生成
    _light = Sprite::create("init/Home/DailyMission/Clear/light.png") ;
...
void DailyMissionClearAnimLayer::createRewardTitle()
{
    if(_all_complate_anim_flg){
        _reward_title = Sprite::create("init/Home/DailyMission/Clear/title_comp.png") ;
        _reward_title->setPosition(convertFromDisplaySize(Point(DISPLAY_WIDTH/2, DISPLAY_HEIGHT*5/6) ,0.5 ,0.5)) ; //画面のちょっと上の位置
        
    }else{
        _reward_title = Sprite::create("init/Home/DailyMission/Clear/title_achieve.png") ;
...
void DailyMissionClearAnimLayer::createChara()
{
    if(_all_complate_anim_flg){
        _chara = Sprite::create("init/Home/DailyMission/Clear/navi2.png") ;
...
//        _chara->setPosition(convertFromDisplaySize(Point(DISPLAY_WIDTH/2, 0) ,0.5 ,0)) ; //下にぴったり
        ...
        _chara = Sprite::create("init/Home/DailyMission/Clear/navi1.png") ;
        _chara->setScale(ratio) ;
...
void DailyMissionClearAnimLayer::createBooty()
{
    ...
    {
        _booty = Sprite::create("init/Home/DailyMission/Clear/reward_l_1.png");
        _booty->setScale(1) ;
    }
    else if (_reward_item_type == "jewel")
    {
        _booty = Sprite::create("init/Home/DailyMission/Clear/reward_l_2.png");
        _booty->setScale(1) ;
    }
    else if (_reward_item_type == "exchange_ticket")
    {
        _booty = Sprite::create("init/Home/DailyMission/Clear/reward_l_3.png");
        _booty->setScale(1) ;
    }
...
void DailyMissionClearAnimLayer::createBootyTitle()
{
    //報酬文字
    string booty_title = "";
    if (_reward_item_type == "chara")
...
void DailyMissionClearAnimLayer::createBg()
{
    //影
...
/*
void DailyMissionClearAnimLayer::popupAction()
{
...
void DailyMissionClearAnimLayer::deleteAction()
{
    auto children = this->getChildren();
...
void DailyMissionClearAnimLayer::setSchedulerForClose(Layer *parent, SEL_SCHEDULE sel){
    _parent = parent;
    _sel_for_close = sel;
}

bool DailyMissionClearAnimLayer::onTouchBegan(cocos2d::Touch *touch, cocos2d::Event *event)
{
    HMSLOG("touch DailyMissionClearAnimLayer!");
...
void DailyMissionClearAnimLayer::onTouchMoved(cocos2d::Touch *touch, cocos2d::Event *event)
{
}
void DailyMissionClearAnimLayer::onTouchEnded(cocos2d::Touch *touch, cocos2d::Event *event)
{
}
void DailyMissionClearAnimLayer::onTouchCancelled(cocos2d::Touch *touch, cocos2d::Event *event)
{
}
