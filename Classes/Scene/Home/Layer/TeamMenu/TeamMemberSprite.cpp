...
TeamMemberSprite::TeamMemberSprite()
:_user_master_id ()
,_friend_id ()
,_friend_card_id ()
,_friend_name ()
...
TeamMemberSprite* TeamMemberSprite::create(json11::Json json, int count){
    
    TeamMemberSprite *sprite = new TeamMemberSprite();
...
bool TeamMemberSprite::init(json11::Json json, int count)
{
...
void TeamMemberSprite::setupData(json11::Json json, int count)
{
    //jsonデータをメンバ変数に設定
...
void TeamMemberSprite::createFriendCard()
{
    //フレンドカードの作成
...
void TeamMemberSprite::showfriendCard()
{
//    //画像のサイズを86×86に統一する処理
...
void TeamMemberSprite::showData()
{
//    //フレンドID
...
//    this->addChild(friend_id, 2);
    ...
    bool is_leader = (_user_master_id==toString(SaveData::loadInt("team_admin_user_master_id"))) ;
    ...
    if(is_leader) {
        up_height = 10 ;
        //スプライト画像の変更
        this->setTexture("init/Home/Friend/popup_leader.png");
        
        Sprite *leader = Sprite::create("init/Team/management/member/leader.png");
        //leader->setScale();
        leader->setPosition(Point(70, this->getContentSize().height-20));
    ...
    level_label1->setPosition(Point(130, 100+up_height));
    ...
    level->setPosition(Point(166, 100+up_height));
    ...
    rank_label->setPosition(Point(130, 74+up_height));
    ...
    name_label->setPosition(Point(230, 86+up_height));
    ...
    login_label->setPosition(Point(560, 100+up_height));
    ...
    login_label2->setPosition(Point(560, 72+up_height));
    ...
    comment_label->setPosition(Point(130, 28+up_height));
    comment_label->enableOutline(Color4B(0, 0, 0, 255), 2);
    this->addChild(comment_label, 2);
}

string TeamMemberSprite::parseLastPlayDate()
{
    //last_play_dateの分解処理
