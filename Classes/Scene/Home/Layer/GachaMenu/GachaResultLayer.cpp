...
bool GachaResultLayer::init()
{
    if ( !Layer::init() )
    {
        return false;
    }
    HMSLOG("GachaResultLayer::init()");
    initEventListener();

    return true;
}

void GachaResultLayer::setBoxGachaFlg(bool box_gacha_flg)
{
    _box_gacha_flg = box_gacha_flg;
}

void GachaResultLayer::setData(int gacha_type,int card_count,int hit_card[],int chara_list_id[],int rare_card[],string card_name_list[],int chara_type[]){
...
void GachaResultLayer::startAnimation(){
    auto normal_scene = (NormalScene *)this->getParent();
    if (!normal_scene) {
        return;
    }
    auto gachaResultAnimationLayer = GachaResultAnimationLayer::create();//リザルトアニメーションの生成
    gachaResultAnimationLayer->setBoxGachaFlg(_box_gacha_flg);
    gachaResultAnimationLayer->setTag(6);
    gachaResultAnimationLayer->setData(_gachatype,_card_count,_hit_card,_rare_card,_chara_type,_chara_list_id);
    gachaResultAnimationLayer->setPosition(convertFromDisplaySize(Point(0, 0), 0, 0));
    SEL_SCHEDULE s = schedule_selector(GachaResultLayer::afterAnimation);
    gachaResultAnimationLayer->setScheduler(this, s);
    normal_scene->addChild(gachaResultAnimationLayer,10);
...
void GachaResultLayer::createBg(){
...
void GachaResultLayer::onEnter(){
    Layer::onEnter();

    //ページタイトルの作成
    //HeaderLayer::setupTitleBar("ガチャ結果", "GachaMenu", this);
    if(_box_gacha_flg==false) {
        HeaderLayer::setupTitleBar("ガチャ結果", "GachaMenu", this);
    } else {
        HeaderLayer::setupTitleBar("BOXガチャ結果", "BoxGacha", this);
    }
...
void GachaResultLayer::afterAnimation(float dt){
    //フレンドスプライトの作成
    initCardDetailCell();
...
void GachaResultLayer::initEventListener()
{
    //タップイベントの設定
    listener = EventListenerTouchOneByOne::create();
    listener->onTouchBegan = CC_CALLBACK_2(GachaResultLayer::onTouchBegan, this);
    listener->onTouchMoved = CC_CALLBACK_2(GachaResultLayer::onTouchMoved, this);
    listener->onTouchEnded = CC_CALLBACK_2(GachaResultLayer::onTouchEnded, this);
    listener->onTouchCancelled = CC_CALLBACK_2(GachaResultLayer::onTouchCancelled, this);
    listener->setSwallowTouches(true) ;
    
    //タップイベントの登録
    _eventDispatcher->addEventListenerWithSceneGraphPriority(listener, this);
}
bool GachaResultLayer::onTouchBegan(cocos2d::Touch *touch, cocos2d::Event *event)
{
...
void GachaResultLayer::onTouchMoved(cocos2d::Touch *touch, cocos2d::Event *event)
{
}
void GachaResultLayer::onTouchEnded(cocos2d::Touch *touch, cocos2d::Event *event)
{
    auto tag = 0;
...
void GachaResultLayer::showDetail(int tag)
{
    auto detail_layer = CardDetailLayer::create(toString<int>(_hit_card[tag]));
    this->addChild(detail_layer, 999999) ;
}

void GachaResultLayer::onTouchCancelled(cocos2d::Touch *touch, cocos2d::Event *event)
{
}

void GachaResultLayer::scrollViewDidScroll(ScrollView *view)
{
    //スクロールしたらボタンタッチをfalseにする
    touchButtonFlg = false;
}

void GachaResultLayer::initCardDetailCell()
{
...
Layer* GachaResultLayer::createList()
{
...
void GachaResultLayer::createScrollView()
{
