...
bool GachaAnimationLayer::init()
{
    if ( !Layer::init() )
    {
        return false;
    }
    HMSLOG("GachaAnimationLayer::init()");
...
void GachaAnimationLayer::setData(int gacha_type){
    _gacha_type = gacha_type ;
}

void GachaAnimationLayer::createBg(){
    //タップ禁止レイヤー生成
...
void GachaAnimationLayer::playAnimation(){

    runAction(Sequence::create(DelayTime::create(0.3f),CallFunc::create(CC_CALLBACK_0(GachaAnimationLayer::gachaAnimation,this)),DelayTime::create(1.7f),CallFunc::create(CC_CALLBACK_0(GachaAnimationLayer::quitAnimation,this)), RemoveSelf::create(), nullptr));

    runAction(Sequence::create(DelayTime::create(0.3f),CallFunc::create(CC_CALLBACK_0(GachaAnimationLayer::gachaSE,this)), nullptr));
}

void GachaAnimationLayer::gachaSE(){
    //ガチャアニメーションエフェクト音(１回)
    CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("download/SE/boss_erase2.mp3",false);
}

void GachaAnimationLayer::gachaAnimation(){
...
//    if(visibleSize.height >1000)animation_size = 2.7;
    ...
    if(_gacha_type == 1){
        ...
        Effect *animation = Effect::create();
        animation->setScale(1.0*ratio,1.2f*ratio);
        animation->setData(this, "init/Home/Gacha/gacha_pay" , 35, Point((main_screen_w / 2) * ratio,(main_screen_size + header_height + footer_height) / 2 * ratio),animation_size, 10);
    }
}

void GachaAnimationLayer::quitAnimation(){
    this->removeFromParent();
}


void GachaAnimationLayer::onEnter(){
    Layer::onEnter();
