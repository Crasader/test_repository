...
MessageTalkSprite* MessageTalkSprite::create()
{
    MessageTalkSprite *sprite = new MessageTalkSprite();
...
bool MessageTalkSprite::init()
{
    if (!Sprite::initWithFile("init/Tutorial/navi1.png"))
    {
        return false;
    }
    HMSLOG("MessageTalkSprite::init()");
...
Label *MessageTalkSprite::createMessageLabel(string message)
{
...
//トーク待機状態にする
void MessageTalkSprite::talkWait(){
...
void MessageTalkSprite::talkStart(string message){
    //SE
    CocosDenshion::SimpleAudioEngine::SimpleAudioEngine::getInstance()->playEffect("download/SE/navi_open.mp3");
...
void MessageTalkSprite::forceAllTalk()
{
    stopActionByTag(TAG_STATE_CHANGE_ACTION) ;
    if(isTalkNow()){
...
bool MessageTalkSprite::isTalkWait(){
    return (talk_state == TalkState::STATE_TALK_WAIT);
}

bool MessageTalkSprite::isTalkNow(){
    return (talk_state == TalkState::STATE_TALK_NOW);
}

bool MessageTalkSprite::isTalkEnd(){
    return (talk_state == TalkState::STATE_TALK_END);
}

//しゃべるアクションを開始
void *MessageTalkSprite::runTalkAction(Label *label, const char message_str[])
{
...
void MessageTalkSprite::showTriangle(){
    //ジャンプし続ける三角
...
DrawNode* MessageTalkSprite::getTriangleShape()
{
