...
bool UserCutInLayer::init()
{
	...
	HMSLOG("UserCutInLayer::init()");
	
	return true;
}
void UserCutInLayer::setData(int chara_id, string user_name, int total_damage, float time)
{
	...
	//------------------カットインレイヤーを構成------------------//
	//フレーム構築
	frame = Sprite::create("init/Map/cutin/bk.png");
	//画像の大きさを調整
	frame->setScale(ratio);
	//配置設定
	float frame_center_width = 320;
	float frame_center_height = 120;
	frame->setPosition(convertFromDisplaySize(Point(frame_center_width, frame_center_height), 0.5, 0));
	//レイヤーの子に設定
	this->addChild(frame);
	...
	charaImg->setPosition(Point(-chara_img_width + 240, chara_img_height));
    ...
    /*
	//背景
	Sprite* skill_name_bg = Sprite::create("init/Map/cutin/bk.png");
	...
	//テキスト
	Label* skill_name_label = HMSFontUtil::createWithTTFBySystemFontArgument(user_name + "の攻撃\n" + to_string(total_damage) + "ダメージ!!!", FONT_NAME_SYSTEM, 40);
	skill_name_label->setPosition(Point(frame->getContentSize().width + 100, frame->getContentSize().height/2));
