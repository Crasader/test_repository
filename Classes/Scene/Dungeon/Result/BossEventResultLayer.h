#ifndef __war__BossEventResultLayer__
#define __war__BossEventResultLayer__
...
class BossEventResultLayer : public Layer
{
private:
    
    enum BossEventResultLayerTag{
        TAG_MENU_ITEM,
    };
...
public:
    ...
    BossEventResultLayer(); //コンストラクタ
    
    static BossEventResultLayer* create(bool winner_flg, string winner_string, string from_page_name = "DungeonScene");
    virtual bool init(bool winner_flg, string winner_string, string from_page_name);

    bool _winner_flg; //勝敗フラグ
    string _winner_string;
    ...
    void showBossEventResultLabel(float dt); //バトル結果のラベル画像の表示
    void showBattleBossEventResult(); //対戦結果の表示
...
#endif /* defined(__war__BossEventResultLayer__) */
