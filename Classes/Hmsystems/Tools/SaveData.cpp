...
void SaveData::saveFloat(const char *key,float value){
    
    UserDefault *userDefalt = UserDefault::getInstance();
    userDefalt->setFloatForKey(key, value);
    
    userDefalt ->flush();
    
}

void SaveData::saveString(const char *key,const string string){
    
    UserDefault *userDefalt = UserDefault::getInstance();
    userDefalt->setStringForKey(key, string);
...
float SaveData::loadFloat(const char *key){
    
    UserDefault *userDefalt = UserDefault::getInstance();
    float _floatvalue = userDefalt->getFloatForKey(key);
    
    return _floatvalue;
}

string SaveData::loadString(const char *key){
