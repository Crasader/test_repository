...
{
    HMSLOG("LabelAttributedBMFont ~LabelAttributedBMFont()");
}
...
void LabelAttributedBMFont::onTouchEnded(cocos2d::Touch *touch, cocos2d::Event *event)
{
    ...
    if (m_iterator == m_pages.end()) {
        HMSLOG("LabelAttributedBMFont : fin");
        return;
    }
    ...
    m_iterator++;
    if (m_iterator == m_pages.end()) {
        HMSLOG("LabelAttributedBMFont : callback");
...
void LabelAttributedBMFont::addKeyWord(string keyword, Color3B fontColor/*, float fontScale*/)
{
	...
	if (keyword == NEWLINE_CHAR) {
		HMSLOG("LabelAttributedBMFont addKeyWord() keyword is newline character!!!");
		return;
	}
...
void LabelAttributedBMFont::removeKeyWord(string keyword)
{
    ...
    HMSLOG("LabelAttributedBMFont not found keyword:%s", keyword.c_str());
